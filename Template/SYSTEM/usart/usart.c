...
u8 usart6_buf[USART_BUFFER_SIZE] = {0};
u32 buf_len = 0;
...
void USART6_Init(u32 baudrate)
{
	...
	GPIO_PinAFConfig(USART6_TX_PORT, GPIO_PinSource9, GPIO_AF_USART1);
	GPIO_PinAFConfig(USART6_RX_PORT, GPIO_PinSource10, GPIO_AF_USART1);

	/*初始化TX引脚*/
	gpio.GPIO_Mode = GPIO_Mode_AF;
	gpio.GPIO_OType = GPIO_OType_PP;
	gpio.GPIO_Speed = GPIO_Speed_100MHz;
	gpio.GPIO_Pin = USART6_TX_PIN;
	GPIO_Init(USART6_TX_PORT, &gpio);
	
	/*初始化RX引脚*/
	gpio.GPIO_Mode = GPIO_Mode_AF;
	gpio.GPIO_PuPd = GPIO_PuPd_NOPULL;
	gpio.GPIO_Pin = USART6_RX_PIN;
	GPIO_Init(USART6_RX_PORT, &gpio);
	...
	USART_ITConfig(USART6, USART_IT_RXNE, ENABLE); //使能串口接受中断
...
void USART6_IRQHandler(void)
{
	...
	{
		usart6_buf[buf_len] = USART_ReceiveData(USART6);
		buf_len++;
		if(buf_len == USART_BUFFER_SIZE)
			buf_len = 0;
	}
